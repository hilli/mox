(() => {
	...
	var parser = {
		...
		Sieve: /* @__PURE__ */ __name((v) => parse("Sieve", v), "Sieve"),
		...
		SieveScript: /* @__PURE__ */ __name((v) => parse("SieveScript", v), "SieveScript"),
...
`));
			...
			(() => {
				let elem = dom.div();
				const reload = /* @__PURE__ */ __name(async (target, p) => {
				...
				const editScript = /* @__PURE__ */ __name(async (target, name) => {
					const content = await check(target, client.SieveScript(name));
					let body;
					let warningsBox;
					const close = popup(dom.div(style({ maxWidth: "60em" }), dom.h1('Edit Sieve script "' + name + '"'), warningsBox = dom.div(), dom.form(/* @__PURE__ */ __name(async function submit(e) {
						e.preventDefault();
						e.stopPropagation();
						...
						await reload(e.target, Promise.resolve());
						close();
					}, "submit"), body = dom.textarea(new String(content), attr.rows("20"), style({ width: "100%", fontFamily: "monospace" })), dom.br(), dom.submitbutton("Save"))));
				}, "editScript");
				const render = /* @__PURE__ */ __name(() => {
					const e = dom.div(dom.table(dom.thead(dom.tr(dom.th("Name"), dom.th("Size"), dom.th("Active", attr.title("Only the active script is executed during delivery.")), dom.th("Created"), dom.th("Updated"), dom.th("Action"))), dom.tbody(sieveScripts.length === 0 ? dom.tr(dom.td(attr.colspan("6"), "(None)")) : [], sieveScripts.map((s) => dom.tr(dom.td(s.Name), dom.td(style({ textAlign: "right" }), formatQuotaSize(s.Size)), dom.td(s.Active ? "\u2713" : ""), dom.td(age(s.Created)), dom.td(age(s.Updated)), dom.td(style({ display: "flex", gap: ".5ex", flexWrap: "wrap" }), dom.clickbutton("Edit", /* @__PURE__ */ __name(function click(e2) {
						editScript(e2.target, s.Name);
					}, "click")), s.Active ? dom.clickbutton("Deactivate", /* @__PURE__ */ __name(function click(e2) {
						reload(e2.target, client.SieveSetActive(""));
					}, "click")) : dom.clickbutton("Activate", /* @__PURE__ */ __name(function click(e2) {
						reload(e2.target, client.SieveSetActive(s.Name));
					}, "click")), dom.clickbutton("Rename", /* @__PURE__ */ __name(function click() {
						let newName;
						const close = popup(dom.div(dom.h1('Rename Sieve script "' + s.Name + '"'), dom.form(/* @__PURE__ */ __name(async function submit(e2) {
							e2.preventDefault();
							e2.stopPropagation();
							await reload(e2.target, client.SieveRenameScript(s.Name, newName.value));
							close();
						}, "submit"), dom.label(style({ display: "block", marginBottom: "1ex" }), dom.div(dom.b("New name")), newName = dom.input(attr.required(""), attr.value(s.Name))), dom.submitbutton("Rename"))));
					}, "click")), dom.clickbutton("Delete", s.Active ? attr.disabled("") : [], attr.title(s.Active ? "The active script cannot be deleted; deactivate it first." : ""), /* @__PURE__ */ __name(function click(e2) {
						if (!window.confirm('Are you sure you want to delete Sieve script "' + s.Name + '"?')) {
							return;
						}
						reload(e2.target, client.SieveDeleteScript(s.Name));
					}, "click"))))))), dom.clickbutton("Add script", style({ marginTop: "1ex" }), /* @__PURE__ */ __name(function click() {
						let name;
						let body;
						const close = popup(dom.div(style({ maxWidth: "60em" }), dom.h1("Add Sieve script"), dom.form(/* @__PURE__ */ __name(async function submit(e2) {
							e2.preventDefault();
							e2.stopPropagation();
							const warnings = await check(e2.target, client.SievePutScript(name.value, body.value));
							if (warnings) {
								window.alert("Saved with warnings: " + warnings);
							}
							await reload(e2.target, Promise.resolve());
							close();
						}, "submit"), dom.label(style({ display: "block", marginBottom: "1ex" }), dom.div(dom.b("Name")), name = dom.input(attr.required(""))), dom.label(style({ display: "block", marginBottom: "1ex" }), dom.div(dom.b("Script")), body = dom.textarea(attr.rows("20"), style({ width: "100%", fontFamily: "monospace" }))), dom.submitbutton("Add"))));
					}, "click")));
					if (elem) {
						elem.replaceWith(e);
					}
