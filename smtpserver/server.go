...
func (c *conn) deliver(ctx context.Context, recvHdrFor func(string) string, msgWriter *message.Writer, iprevStatus iprev.Status, iprevAuthentic bool, dataFile *os.File) {
	...
	processRecipient := func(rcpt recipient) {
		...
		for _, a := range la {
			...
			// mailboxDestined records where the message would have gone if
			// introbox/rejects hadn't rerouted it. An explicit Sieve fileinto
			// supersedes that routing, so the hint no longer applies.
			targetMailboxDestined := a.mailboxDestined
			if sieveDecision != nil && sieveDecision.Mailbox != "" && sieveDecision.Mailbox != a.mailbox {
				targetMailbox = sieveDecision.Mailbox
				targetMailboxDestined = ""
			}

			var delivered bool
			a.d.acc.WithWLock(func() {
				if err := a.d.acc.DeliverMailbox(log, targetMailbox, targetMailboxDestined, a.d.m, dataFile); err != nil {
